...
def syntax():
    ...
    print(
        "Colors and options can be provided in any order. At least one color is required."
    )
    ...
    print(
        "	--color				Set strip to single color, only first color will be used if multiple are provided. This is the default."
    )
    print("	--morph				If multiple colors are provided, morphs strip between colors.")
    print(
        "	--flow				Like morph, but the colors smoothly flow in from one side of the strip to the other."
    )
    ...
    print(
        "	--sduration=TIME	How long an individual strobe flash should last. Used with --strobe."
    )
    print("")
    ...
    if color[0] == "#":  # Hex code
        if len(color) != 7:
            ...
            if max(r, g, b) > 255 or max(r, g, b) < 0:
                print(
                    "Error: not a valid hex code (",
                    color,
                    "). Values must be in the range 00-FF",
                )
                syntax()
    elif color[0:4] == "rgb(":
        if color[-1] != ")" or color.count(",") != 2:
            print(
                "Error: not a valid rgb color (",
                color,
                "). Should match pattern: rgb(rrr,ggg,bbb)",
            )
            syntax()
        ...
        try:
            ...
            print(
                ...
                color,
                "). Not a valid hex code (#RRGGBB), rgb color (rgb(rrr,ggg,bbb)), or HTML WebColor name.",
            )
            syntax()
    return (g, r, b)
...
def send(g, r, b):
    for stick in sticks:
        if stick.get_led_count() == -1:
            ...
            stick.set_led_data(
                0,
                [int(g * bright), int(r * bright), int(b * bright)]
                * stick.get_led_count(),
            )
        ...
        for color in colorlist:
            ...
            while time() - start < duration:
                ...
                (g, r, b) = [
                    ...
                    for a, b in zip(curcolor, targetcolor)
                ]
                if flow:
                    push(g, r, b)
                else:
...
def blink(colorlist, strobe):
    while True:
        for color in colorlist:
            ...
            send(g, r, b)
            if strobe:
                sleep(max(0.02, blink_duration))
                send(0, 0, 0)
            sleep(max(0.02, duration))
        if not loop:
            break
...
def main():
    ...
    if action == "morph":
        ...
        if not loop:
            ...
            )  # Duplicate the last color if we're not looping so it gets a chance to cover the entire strip.
