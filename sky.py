...
def main():
    ...
    sun1 = [0.11, 0.9, 1]  # These are in hsv format
    sun2 = [0.14, 0.98, 1]  # Color of the sun
    sky1 = [0.6, 0.7, 0.7]  # Color of the sky
    sky2 = [0.45, 0.8, 0.65]  # It'll shift between the two colors.
    ...
    for (
        arg
    ) in sys.argv:  # Messy way to have it mildly configurable on the command line.
        if arg == "not_cloudy":
            cloudy = False
        elif arg == "cloudy":
            cloudy = True
    ...
    while True:
        data = []
        for i in range(cnt):
            sky_color = math.sin((time.time()) * sky_speed + i) / 2 + 0.5
            sun_color = (
                math.sin((time.time()) * sun_speed + i * sun_speed * 1.5) / 2 + 0.5
            )
